...
As shown in Figure~\ref{fig:QUIC_handshake_vs_TCP-TLS_handshake}, one of the core innovations of \texttt{QUIC} is to combine the transport and security layer handshakes thus reducing latency of the first \texttt{HTTP} request.
 ...
 % MAYBE TODO: mention possible amplification attacks of \texttt{QUIC} (https://blog.cloudflare.com/the-road-to-quic/) 
  ...
  In addition, encrypted \texttt{QUIC} headers do not provide scientific information for network engineers about the traffic patterns~\cite{2017-12-18-transport-header-encryption}.
  ...
  \item \simon{TODO READ and INCLUDE https://blog.cloudflare.com/http-3-vs-http-2/}
  ...
  \item \simon{TODO multiplexed streams}
 
  \item \simon{TODO loss detection with signaling (using new seq number)}
  ...
  In particular, the study revealed that in some cases the time required to download contents of Google's website was reduced by one second~\cite{chromium_blog_about_quic}.
