...
For example, usually, localhost connections use a maximum transmission unit (MTU) of around 65000 bytes, while in the actual Internet, it is usually around 1500 bytes.
 ...
 The overhead measurment results are located in \S~\ref{physical_setup_subsection}.
...
I achieved this by implementing logic that enacted a \it{null} encryption in \texttt{ngtcp2} (see Figure~\ref{fig:my_implementation_of_cryptographic_procedures_of_quic}).
...
My final implementation of null encryption has room for improvements.
...
\subsection{MTUs}
   \simon{TODO introduce/refresh MTUs}
    ...
    Table~\ref{fig:Impact_of_Jumbo_frames_for_ngtcp2_throughput} summarises tests that measure the impact of different MTU sizes.    
    In particular, this experiment measures \texttt{QUIC} throughput between the server and the client when they are pinned to two different cores on the same machine, and their packets travel via an intermediate machine (as demonstrated in Figure~\ref{fig:Physical_testing_environment}).
    ...
    According to Table~\ref{fig:Impact_of_Jumbo_frames_for_ngtcp2_throughput}, \texttt{QUIC} throughput can be improved by increasing the size of Ethernet frames.
    \simon{Explain why there is throughput improvement}
    However, this improvement is only applicable when transferring large (e.g., 1 GByte) files.
    Furthermore, MTUs above 1500 bytes are not prevalent on the Internet~\cite{why-is-ethernet-mtu-1500}.
...
With a large number of clients, one can offer the server a variety of packets it can process}
...
% TODO
\simon{TODO: explain function of UDP throughput in time (see Figure~\ref{fig:UDP_throughput_in_time}}
    \begin{figure}[H]
    \centering
