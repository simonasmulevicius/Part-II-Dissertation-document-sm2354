...
In order to measure performance of \texttt{QUIC}, I had to select suitable network simulation tools.
...
For instance, packets sent via localhost are simply memory-copied from one place on the machine to another meaning that \texttt{QUIC} packets would not actually leave the machine.
...
For example, usually, localhost connections use maximum transmission unit (MTU) of around 65000 bytes while in the real network it is usually around 1500 bytes.

 Alternatively, I could have forced packets to leave the testing machine but send them via loopback cable to the same machine on different interface. 
 That would have been still a relatively simple setup. 
 Moreover, I would have allowed me to inspect behaviour of NICs.
 ...
 As a result, I only used this configuration to test the correct partial setup required for the next simulation environment.
 
 To elevate these issues, I used a separate intermediate dedicated machine to introduce traffic perturbations.
 Such testing environment has already been used in the previous studies~\cite{Making_QUIC_Quicker}.
 However, it was rather complicated set-up process to correctly configure the packet forwarding and network parameters.
 But as I was not restricted by hardware, I used this option for the final performance analysis tests.
