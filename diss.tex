...
As \texttt{QUIC} builds on top of \texttt{UDP}, then we can assume that \texttt{UDP}'s throughput is a theoretical maximum \texttt{QUIC} throughput.
...
An important point to mention is that \texttt{UDP} does not guarantee reliable packet delivery so the sender and receiver would see different throughput values.
...
\texttt{Wireshark} is a conventional tool used to inspect captured network packets (see Figure~\ref{fig:Wireshark_screenshot}). 
...
I used \texttt{Wireshark} in an ethical manner meaning that I only used it to analyse traffic between the dedicated interfaces.
In other words, as demonstrated in Figure~\ref{fig:Wireshark_separation}, I did not capture packets from the college network because testing environment used separate network interfaces. 
    ...
    \caption{\texttt{Wireshark} was not used to capture packets from the college network}
