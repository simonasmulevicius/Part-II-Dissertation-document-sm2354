...
\subsection{Previous Attempt to Improve \texttt{HTTP} by Using \texttt{HTTP/2.0} and \texttt{SPDY}}
...
Then, now obsolete \texttt{SPDY} protocol was used as a blueprint when building the next-generation version of \texttt{HTTP} \cite{bib_SPDY_vs_HTTP2}.
...
However, attempts to roll out new and experimental versions of \texttt{TCP} were mostly unsuccessful \cite{bib_Netdev_0x13_QUIC_Tutorial}.
% TODO what TCP improvements were not successfully deployed?
% TODO: mention contribution of IPv4 to the proliferation of middleboxes
% TODO rewrite this paragraph with references

% TODO: Person X in conference Y said these ideas
...
Another reason for the slow roll-out was that \texttt{TCP's} functionality is implemented in the kernel, meaning that in order to add changes to \texttt{TCP}, one would need to change a large number of different operating systems.
...
\texttt{HTTP over QUIC} layer (which is now called \texttt{HTTP/3} \cite{HTTP_3_the_past_the_present_and_the_future}) is smaller than analogous \texttt{HTTP/2} layer because stream multiplexing and connection management tasks are delegated to \texttt{QUIC}.
...
\begin{itemize}
 ...
 % TODO: 
 %more mobile devices than before - as I have already mentioned in the Introduction, more than 90% of all Internet users use mobile devices to connect to the Web \cite{bib_number_of_mobile_users}
 % mobile devices -> can change location and Internet access point
 % As TCP's connection is tied to a particular IP address, TCP would need to establish new connection every time network access point is changed  \cite{PollardBarry2019HiAP}
 % TCP is old - assumed that IP address wouldn't change frequently  \cite{PollardBarry2019HiAP}
 % This means that TCP would spend more time in this "slow-start" phase
 % In contrast, QUIC has explicit connection ID which does not depend on the IP address > hence, clients can migrate 
...
Machine A in Figure~\ref{fig:Physical_testing_environment} has four physical cores (see Figure~\ref{fig:memory_topology}).
...
Robert Love refers to this technique of assigning particular processes to particular cores as \texttt{hard CPU affinity} \cite{CPU_Affinity}.
    ...
    \begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\textwidth]{figs/memory_topology.png}
    \caption{Memory topology of machine A (Figure generated by \texttt{lstopo} command)}
