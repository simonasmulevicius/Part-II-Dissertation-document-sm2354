...
\bigskip
\leftline{Signed \textit{Simonas Mulevicius}}

\medskip
\leftline{Date \textit{13th May 2021}}
...
Project Title:      & \bf Analysing the behaviour and \\
                    & \bf performance of \texttt{QUIC} \\
Examination:        & \bf Computer Science Tripos -- Part II, July 2021  \\
Word Count:         & \bf 11326\footnotemark[1] \\
Line Count:         & \bf 5805\footnotemark[2] \\
...
\footnotetext[2]{More details about the code composition are in \S~\ref{repository_overview_section}.}
...
% % OLD "Special difficulties" section
...
% I spent more than one month working on the remote set-up of both the main pair of testing machines and the backup pair.
...
In order to measure the impact of cryptography for \texttt{QUIC} performance, I extended one implementation of \texttt{QUIC} (i.e., \texttt{ngtcp2}) to support \textit{Null} encryption.
...
An analysis of the \texttt{QUIC} protocol (also involving \texttt{ngtcp2}) was published in 2018~\cite{overview_of_the_QUIC_protocol}.
...
By extending one of the existing implementations of \texttt{QUIC} to support unencrypted packets, I was able to increase \texttt{QUIC} throughput by about 200-300\%.
In addition, I replicated \texttt{QUIC} performance analysis tests by measuring the impact of additional packet delay, packet loss, and packet re-ordering for \texttt{QUIC} throughput.
For this reason, I configured a precise testing environment.
...
%As Robin Marx wrote, HOL could have a detrimental effect on user experience when sequentially rendering web pages that have dynamically generated content or contain large embedded objects~\cite{head-of-line-blocking-in-quic-and-http-3-the-details}.
...
%For example, the transfer of files 3 and 8 can not be started until the blocking resources (i.e. files 2 and 7) are sent. 
    ...
    \begin{figure}[!htb]
    \centering
    \includegraphics[width=0.8\textwidth]{figs/Head_of_line_blocking_of_HTTP1_1.png}
...
% TODO: what does it mean for QUIC streams not to be ordered
    ...
    \begin{figure}[!htb]
    \centering
    \includegraphics[width=0.7\textwidth]{figs/Layers_in_the_protocol_stack.png}
...
Similarly, according to Cloudflare\cite{radar_cloudflare}, at the same time 11\% of traffic used \texttt{HTTP/3}.
...
\section{Software Engineering Techniques and Tools} \label{Software_Engineering_Techniques_and_Tools}
    ...
    %(see Figure~\ref{fig:Trello_board}).
...
[top level folder
  ...
  [Part-II-dissertation(4758)
    [testing-scripts(4441)
    ]
    [testing-results(0)
    ]
    [testing-libraries(317)
    ]
  ]
...
% nghttp2: initial: 5528bb62e9cf23212091a4384e50a690f5b5e96f
% nghttp2: final: 8f01fc4809520c97e41c0f3f47fb7a2e716fca61
% git diff --shortstat 5528bb62e9cf23212091a4384e50a690f5b5e96f 8f01fc4809520c97e41c0f3f47fb7a2e716fca61

% ngtcp2: initial: 2624d0265719720b2af48cebe3dee7f66c0b13ba
% ngtcp2: final:   855b3a270f094abb0053f3ffd04b09319fcd8dfc
% git diff --shortstat 2624d0265719720b2af48cebe3dee7f66c0b13ba 855b3a270f094abb0053f3ffd04b09319fcd8dfc
...
However, in the \textit{null} encryption scenario, these additional fields are not required, but my implementation of disabling encryption does not use them, meaning that 16 unused bytes are transferred in a single \texttt{QUIC} packet.
...
\subsection{GSO}
    Generic Segmentation Offload (GSO) is a software technique that groups system calls issued by user-space applications to send \texttt{UDP} packets~\cite{segmentation-offloads, Tuning_10Gb_network_cards_on_Linux, segmentation-offloading-with-wireshark-and-ethtool}.
...
\hline
\multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}Requested \\ file size\end{tabular}} & \multicolumn{1}{c|}{\multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}MTU \\ (Bytes)\end{tabular}}} & \multicolumn{2}{c|}{\begin{tabular}[c]{@{}c@{}}Completion  time (s)\end{tabular}}                            & \multicolumn{2}{c|}{\begin{tabular}[c]{@{}c@{}}Throughput (Mbits/s)\end{tabular}}                            \\ \cline{3-6} 
...
To be precise, the testing environment used 10 Gigabit Ethernet links, and the end-to-end delay between the client and the server was $0.262 \pm 0.065$ ms (see \S~\ref{physical_setup_subsection}), meaning that the window size was at least $10^{10} \times 2.62 \times 10^{-4}$~bytes $= 2.62$~Mbytes.
...
\begin{tabular}{|r|r|r|r|r|r|}
\hline
\multicolumn{1}{|c|}{\begin{tabular}[c]{@{}c@{}}Requested\\ file size \\ (Mbytes)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Number of\\ parallel \\ clients\\ (1 request\\ per client)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Average\\ total\\ throughput\\ measured\\ on the\\ receiver's\\ side (Mbit/s)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Standard \\ deviation\\ of the\\ throughput\\ sample \\ side (Mbit/s)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Average\\ total\\ completion\\ time (ms)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Standard \\ deviation  of\\ the completion\\ time sample\\ side (ms)\end{tabular}} \\ \hline
...
% \textit{Null} encryption mechanism of  \texttt{MsQuic} works as follows.
...
\hline
\multicolumn{1}{|c|}{\begin{tabular}[c]{@{}c@{}}Requested\\ file size \\ (Mbytes)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Additional\\ delay on the \\ intermediate \\ machine (ms)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Average total\\ throughput\\ measured on \\the receiver's\\ side (Mbit/s)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Standard \\ deviation of \\ the throughput\\ sample\\ side (Mbit/s)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Average\\ total\\ completion\\ time (ms)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Standard \\ deviation \\of the \\completion\\ time sample\\ side (ms)\end{tabular}} \\ \hline
1                                                                                               & 0.0                                                                                                                    & 670.38                                                                                                                                     & 35.15                                                                                                                             & 11.97                                                                                                 & 0.67                                                                                                                                    \\ \hline
...
\begin{tabular}{|r|r|r|r|r|r|}
\hline
\multicolumn{1}{|c|}{\begin{tabular}[c]{@{}c@{}}Requested\\ file size \\ (Mbytes)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Probability \\ of packet \\ loss\\ (\%)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Average\\ total\\ completion\\ time (ms)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Standard \\ deviation  of \\ the completion\\ time sample\\ side (ms)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Average total\\ throughput\\ measured on \\ the receiver's\\ side (Mbit/s)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Standard \\ deviation of \\ the throughput\\ sample\\ side (Mbit/s)\end{tabular}} \\ \hline
1                                                                                               & 0.0000\%                                                                                             & 12.06                                                                                                 & 0.51                                                                                                                                & 665.01                                                                                                                                     & 26.98                                                                                                                             \\ \hline
...
\chapter{\texttt{QUIC} packets captured with \texttt{Wireshark}} 
    \begin{figure}[!htb]
    \centering
...
\chapter{Separation of testing and college networks} 
    \begin{figure}[!htb]
    \centering
...
\chapter{Evolution timeline of network technologies} 
    \begin{figure}[H]
    \centering
    % \includegraphics[width=0.7\textwidth]{figs/cf-secure-web-timeline-1.png}


% \label{preparation_script_from_zshrc}
...
\caption[Evolution timeline of the related network technologies]{Evolution timeline of related network technologies. This diagram is taken from \texttt{Cloudflare}'s blog~\cite{http-3-from-root-to-tip}.}
...
\chapter{GSO impact for \texttt{QUIC} throughput}
\begin{table}[!htb]
\begin{tabular}{|r|r|r|r|r|r|}
\hline
\multicolumn{1}{|c|}{\begin{tabular}[c]{@{}c@{}}Is GSO \\ enabled\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Requested\\ file size \\ (Mbytes)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Average\\ total\\ completion\\ time (ms)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Standard \\ deviation\\ of the \\ completion\\ time sample\\ side (ms)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Average\\ total\\ throughput\\ measured\\ on the\\ receiver's\\ side (Mbit/s)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Standard \\ deviation\\ of the\\ throughput\\ sample\\ side (Mbit/s)\end{tabular}} \\ \hline
no                                                                              & 1                                                                                              & 27.54                                                                                                 & 0.41                                                                                                                                & 290.62                                                                                                                                     & 4.29                                                                                                                              \\ \hline
