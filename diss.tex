...
the web is the Internet, yet earlier http do not have the rich features people now desire (you might enumerate a sshort list of things the http and tcp protocols don't do); QUIC provbides many features but as a new protocol is far from the high-performance transport we have come to need and expect in the Internet \\
\textit{What are you doing / did you do?}\\
Experimental studty of a QUIC implementation comparing the behaviour of two implementations, and identifying core contributors for future improvement and optimization 
...
\caption[Evolution timeline of the related network technologies]{Evolution timeline of the related network technologies. This diagram is taken from \texttt{Cloudflare}'s blog~\cite{http-3-from-root-to-tip}.\simon{UNIMPORTANT TODO: add dates on the timeline axis}}
...
As shown in Figure~\ref{fig:QUIC_handshake_vs_TCP-TLS_handshake}, one of the core innovations of \texttt{QUIC} is to combine the transport and security layer handshakes thus reducing latency of the first \texttt{HTTP} request.
 ...
 % MAYBE TODO: mention possible amplification attacks of \texttt{QUIC} (https://blog.cloudflare.com/the-road-to-quic/) 
  ...
  Furthermore, Langley et. al. found that \texttt{YouTube's} servers used 350\% more \texttt{CPU} resources when delivering content via \texttt{QUIC} connections as opposed to using \texttt{TCP} with \texttt{TLS}~\cite{QUIC_SERVER_REQUIRES_3-5_more_CPU_resources}.
  ...
  This is a conventional mechanism also seen in \texttt{TCP} that prevents the slower receiver from being flooded with the sender's messages~\cite{ietf-quic-transport-draft-32}.
    ...
    It is important to mention that \texttt{QUIC} uses unique monotonically increasing packet sequence numbers~\cite{UnderstandQUIC},~\cite[Chapter~4.2]{ietf-quic-recovery-32}.
    As a result, lost \texttt{QUIC} frames are retransmitted over \texttt{QUIC} packets with different sequence numbers~\cite[Chapter~4]{ietf-quic-recovery-32}.
    In contrast, \texttt{TCP} reuses packet numbers for retransmissions~\cite[Chapter~4]{ietf-quic-recovery-32}.
    ...
    This concept is visualised in Figure X.
    This diagram demonstrates that \texttt{QUIC} can cope better with repeated packet loss than \texttt{TCP}~\cite{UnderstandQUIC}.
  ...
  In case of the packet loss, \texttt{QUIC} passes other successfully received packets to the application layer provided that a missing packet is from another stream~\cite{head-of-line-blocking-in-quic-and-http-3-the-details}.
  


  \simon{TODO loss detection with signaling (using new seq number)????}
   ...
   The same \texttt{QUIC} congestion control algorithm is applied over all \texttt{QUIC} streams, meaning that a single lost packet on one stream would likely have an impact on the throughput of all streams, but a lost packet of one stream would not block the other streams~\cite{head-of-line-blocking-in-quic-and-http-3-the-details, UnderstandQUIC}.
   Similarly, round-trip time estimations are also shared among different streams~\cite[Chapter~4.2]{ietf-quic-recovery-32}.
  ...
  \item
    ...
    The effect of the reduced latency of \texttt{QUIC} is apparent.
  For example, engineers in Google discovered that, when compared with \texttt{TCP}, \texttt{QUIC} substantially improved user experience for the customers that used the low quality Internet links~\cite{chromium_blog_about_quic}.
...
Header encryption hides some privacy information that could be otherwise used to correlate traffic of a single user~\cite{2017-12-18-transport-header-encryption, the-road-to-quic}.


\begin{itemize}
  \item  \simon{TODO mention that the packet header encryption is separate from payload encryption}
  \item  \simon{TODO explain why do we need this separation}
  
 \item  \simon{TODO present long and short headers give a list of fields (maybe to Appendix)}
  
  \item  \simon{ TODO mention that packet numbers are encrypted, but connection ID is not}
\end{itemize}
...
\subsection{nuttcp}
\simon{ According to https://fasterdata.es.net/performance-testing/network-troubleshooting-tools/nuttcp/ , nuttcp is better than iperf for high UDP throughput measurements. }

\simon{Similarly, another source (https://wiki.geant.org/display/public/EK/Nuttcp) also said that nuttcp provides more stable results when analysing UDP}
...
In order to measure performance of \texttt{QUIC}

There were several ways I could have measured 
...
In this chapter I describe the actual software engineering tasks I completed to measure \texttt{QUIC} performance consistently and correctly.
...
\simon{\url{https://tools.ietf.org/html/draft-ietf-quic-tls-19} section-5.4 describes header encryption}
...
In this project I simulated offloading of cryptographic \texttt{QUIC} functions and I measured impact of this improvement.
