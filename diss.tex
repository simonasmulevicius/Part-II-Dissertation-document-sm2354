...
\simon{TODO: bring those numbers here}
...
At first, the sender encrypts the \texttt{QUIC} packet payload by using both the plain text payload and 16-byte long authentication tag~\cite[Section 5]{ietf-quic-tls-32}. 
% The authentication tag depends on the Destination connection ID, Initiail Salt and other cryptographic parameters.
% In this stage, the produced  cyphertext  is 16 bytes longer than the provided plaintext because the cyphertext has to accomodate both the plaintext and the authentication tag~\cite[Section 5.3]{ietf-quic-tls-32}
% 2. Encrypt packet header with a 5 byte long mask.
%  \texttt{QUIC} packet protection key is derived in a different way than header protection key so they are different
% Header encryption is performed by \texttt{XORing} relevant packet header fields (e.g., packet number) with the associated header encryption key.
% 3. Send encrypted packet 
% 4. Receiver has to do the steps in reverse - decrypt header and then payload


\simon{Describe Figure~\ref{fig:Cryptographic_procedures_of_quic}}


\simon{\url{https://tools.ietf.org/html/draft-ietf-quic-tls-32} section-5.4 describes header encryption}

    \begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figs/Cryptographic_procedures_of_quic.png}
...
\simon{Explain my solution}

% 1. Short circuit payload encryption
% 2. Header encryption key is generated by sampling first few hundred bytes of the encrypted packet
% 3. In order to avoid this and ensure that packets are completely transparent, I also disable header encryption.
% However, headers  are still XORed with a fixed 5byte long mask
% But my implementation avoids sampling procedure?? (to check) required to generate the header encryption key in the first place
% Alternatively, I could have even removed the function calls that are used to encrypt QUIC packet headers.
% But QUIC is a complex protocol (people complain about its complexity), so removal of these functions could have had any additional unintented side effects that could have lead to the protocol errors.
% Headers are negligible compared with the payloads so futrther performance optimisations would have provided only limited performance improvements.
...
In essence, null encryption is a technique that effectively turns off encryption by encrypting/decrypting the data by ``XOR"ing the data with a key made entirely of zeroes (which leaves the data unchanged). 
    ...
    \begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figs/my_implementation_of_cryptographic_procedures_of_quic.png}
...
% TODO
\simon{TODO: mention improved null encryption (e.g., add a short code snippet demonstrating existing memcopy vs my naive implementation)}

% TODO
\simon{TODO: mention that the performance analysis I described before allowed me to identify potential bottlenecks of my implementation.
...
\simon{Rephrase sentence before}
...
\simon{TODO: add information about the capacities of Ethernet links}
    ...
    The purpose of these addresses is to prevent experiments from interfering with the college network and prevent interference of external packets in the experiments.
    ...
    In both cases, throughput was measured on the receiver side.
    ...
    % TODO
    \simon{TODO: add a table of results showing that lower RTT of loopback cables increased throughput of ngtcp2-h2load by about 10\%}
    ...
    Large segmentation offload and \texttt{TCP} segmentation offload mechanisms were in place on machine A. 
    \simon{TODO: check which ones, because I disabled LRO}
...
%TODO
\simon{TODO: Ubuntu 20.04 has a different mechanism  to disable IPv6 -- check that it is still working}
...
As far as testing conditions are concerned, the intermediate machine was not configured to introduce additional explicit traffic perturbations.
    ...
    \end{figure}
% TODO
\simon{TODO: Am I talking here about iperf3 client-server pair, or is it the ngtcp2 pair?}
...
\end{table}
  ...
  \item 
  \simon{TODO show that Jumbo frames are not accepted in the wild Internet / TODO state that MTUs above 1500 bytes are not prevalent on the Internet}
...
Breno Henrique Leitao in his work suggested using~\cite{Tuning_10Gb_network_cards_on_Linux} Interrupt Request Queue (IRQ) Balancing to improve the general throughput of the testing system.
...
The suggestion was to delegate the processing of certain interrupts to specified processors hoping that this would improve the performance of cores that were designated for the \texttt{QUIC} tests.
...
One plausible explanation for this insignificant improvement could be that the testing environment used an already saturated network link.
In other words, the measured maximum available throughput before and after the experiment was around 9.8Gbits/second, while the maximum theoretical limit of the link was 10Gbits/second. 
...
1                                                                                                & 1                                                                                                                          & 2607.36                                                                                                                                         & 141.00                                                                                                                           & 3.32                                                                                                     & 0.15                                                                                                                             \\ \hline
...
\simon{Wait a minute, with the new tests, these results don't hold any more!}

However, for very large files this effect is inverted -- additional concurrent \texttt{iperf3} clients decreased \texttt{UDP} throughput measured on the server-side.
...
With a large number of clients, we can offer the server a variety of packets it can process}
...
\hline
\multicolumn{1}{|c|}{\begin{tabular}[c]{@{}c@{}}Requested \\ file size \\ (MBytes)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Number of \\ parallel \\ clients \\ (1 request \\ per client)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Average\\ total \\ throughput \\ measured \\ on the \\ receiver's \\ side (MBit/s)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Standard \\ deviation \\ of the \\ throughput \\ sample \\ (MBit/s)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Average \\ total \\ completion \\ time (ms)\end{tabular}} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Standard \\ deviation \\ of the\\ completion \\ time \\ sample (ms)\end{tabular}} \\ \hline
1                                                                                                & 1                                                                                                                          & 2524.38                                                                                                                                         & 874.14                                                                                                                           & 2.53                                                                                                     & 1.16                                                                                                                             \\ \hline
...
In addition, the transfer of large files allows \texttt{TCP} to amortise the overhead of initial \texttt{TCP} handshake.
Hence, when sending 1GByte of data, \texttt{TCP} was able to approach the maximum link capacity that was 10~Gbits/s.


\simon{describe the function of TCP throughput in time (see Figure~\ref{fig:TCP_throughput_in_time})}


\simon{First 4 rows of the table - there are 4 different experiments with different throughputs (differ by at least the factor of 2), and the average completion time of all of them was around 10ms. Hence, this example demonstrates that iperf3 does not have sufficient resolution to capture and visualise the effect of a slow start. }

% TODO
\simon{TODO: comparison of TCP vs UDP}

\simon{According to https://support.cumulusnetworks.com/hc/en-us/articles/216509388-Throughput-Testing-and-Troubleshooting, TCP might have better offloading support than UDP, meaning that their throughputs may not be comparable}
...
\end{tabular}
    ...
    \simon{Still using MBytes/second. It should be MBits/second!}.
...
\simon{It might be an interesting section to combine all of the performance-boosting ideas (GSO, null-crypto, Jumbo frames, large frame sizes) to create an ngtcp2 version with the highest throughput}
