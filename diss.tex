...
[top level folder
  [Original encrypted stack: {openssl, \textbf{ngtcp2}, nghttp2, nghttp3}
  ]
  [unencrypted\_stack
...
I forked part of networking stack required to run \texttt{ngtcp2}. 
In particular, I worked on a separate \texttt{OpenSSL}(embedded security layer), \texttt{ngtcp2} (transport layer), \texttt{nghttp2} and \texttt{nghttp3} (application layers) 

\simon{Initial code base is spilt into multiple repositories because each is implemented in modular fashion (e.g. \texttt{OpenSSL} code could be used in other protocols or algorithms)}

\simon{ngtcp2 uses code from  \texttt{Openssl} repository. Similarly, nghttp2 uses code from both \texttt{OpenSSL},  \texttt{ngtcp2} and  \texttt{nghttp3} because it contains a performance analysis tool called h2load that measures \texttt{QUIC} throughput.}
...
\simon{Somehow transition to this saying that the most work was done in \textbf{unencrypted\_stack/ngtcp2}, \textbf{unencrypted\_stack/nghtt2} and \textbf{testing-scripts} sections}
I added support of null encryption in the cloned repository of \texttt{ngtcp2}.
...
\simon{Mention that, using the provided instructions, I also added build scripts to automate the process. Furthermore, this ensured that the same build flags were used consistently}

\simon{Mention where unit and integration tests are placed}


\simon{testing-scripts folder contains several different automated evaluation tests.
For example, it contains \texttt{iperf} tests that were used to measure \texttt{TCP} and \texttt{UDP} throughput.
Some of the tests used ssh commands to control the intermediate machine}
\simon{I moved several common testing functions to testing-libraries folder}



\simon{Results of several manually completed tests are stored in testing-results folder}

\simon{I used original encrypted stack during the development process to compare its behaviour with my implementation (stored in unencrypted\_stack)}

\simon{There is a list of other evaluation tools (e.g., FlameGraph, iperf, MsQuic) that I used for the QUIC behaviour analysis}
...
\simon{TODO mention ping test}
\simon{TODO state that it configures virtual namespaces to separate server from client}
\simon{TODO add scripts to appendix}

The automatic preparation scripts improved development speed because certain repetitive tasks were automated.
Furthermore, the setup of the testing environment became more predictable allowing performance tests to generate more reproducible results.
