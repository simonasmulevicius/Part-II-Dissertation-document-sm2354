...
\texttt{Hyper-threading} (more generally known as \texttt{simultaneous multi-threading}) is a thread allocation technique that allows two threads to run on the same physical core at the same time~\cite[page~ 23]{hyperthreading_book}.
...
\subsection{GSO}
\begin{itemize}
  \item \simon{TODO introduce GSO (General Segmentation Offload)}
  
  
  \item \simon{TODO add a chart showing GSO's impact on throughput}
  
  
  \item \simon{TODO mention when GSO is relevant}
  
  \item \simon{TODO state that, GSO is used in all the remaining tests by default because GSO provided great performance improvements}
\end{itemize}

\subsection{MTUs}
\begin{itemize}
  \item \simon{TODO introduce/refresh MTUs}
  \item \simon{TODO demonstrate the Jumbo frames impact for iperf/\texttt{QUIC} throughput}
