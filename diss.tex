...
% TODO add references !!!!
QUIC is a next-generation transport layer protocol built on top of UDP.
Figure~\ref{fig:QUIC_network_stack} shows network stack of QUIC.
QUIC aims to solve some of the design problems which are present in the TCP transport layer protocol.
...
% TODO Mention fast TLS1.3 handshake
...
%TODO justify - mention SPIDY and TCP.2??
...
Consequently, these premature optimisations interfered with TCP protocol's correctness - NATs rejected connections that used non-standard TCP packets.
...
\subsection{QUIC's header format} \label{subsection_QUIC_header_format}

\begin{itemize}
  \item TODO mention that there packet header encryption is separate from payload encryption
...
% This is subsection see Appendix~\ref{referencedAppendixTag}. 

\subsection{Why are there so many implementations of QUIC?}


% TODO user space vs kernel space
% TODO add a list of QUIC implementations

As I have already mentioned in the Introduction section, QUIC \\
TODO 
...
Later on, I figured out that there is QUIC benchmarking tool called \texttt{h2load} \footnote{https://github.com/nghttp2/nghttp2/tree/quic}, which is compatible with \texttt{ngtcp2}.
Moreover, h2load is an experimental tool written by Mr Tatsuhiro  Tsujikawa (the co-author of \texttt{ngtcp2}).
...
To measure QUIC performance under different conditions, we need to add network emulator between the QUIC client and the QUIC server.
